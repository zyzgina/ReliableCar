...
import com.kaopujinfu.appsys.thecar.adapters.GpsAdapter;
 ...
 */
public class GpsListActivity extends BaseNoScoActivity implements View.OnClickListener {
    ...
    public void initView() {
        ...
        gpsAdapter = new GpsAdapter(this, bindingsList);
        gpsAdapter.setUpdateBindingUIListener(new GpsAdapter.UpdateBindingUIListener() {
            @Override
            public void updateUI(int num) {
                newBindingsSelectNum.setText("已选择" + num + "个任务");
            }
        });
        bindingsList.setAdapter(gpsAdapter);
    ...
    private void initDate() {
        HttpBank.getIntence(this).httpGPSLIST(new CallBack() {
            @Override
            public void onSuccess(Object o) {
                LogUtils.debug("获取数据:" + o.toString());
                refreshLayout_bindings.finishRefreshing();
                GPSBean gpsBean = GPSBean.getGPSBean(o.toString());
                if (gpsBean != null && gpsBean.getResult() == 1) {
                    if (page == 1) {
                        IBaseMethod.jumpCountdown(IBase.TIME_REFERSH, handler);
                        gpsAdapter.clearDate();
                    }
                    if (gpsBean.getData() != null)
                        gpsAdapter.setListBean(gpsBean);
                    if (gpsAdapter.getGroupCount() == 0) {
                    ...
                    bindingsNoData.setVisibility(View.VISIBLE);
                    if (gpsBean != null)
                        IBaseMethod.showToast(GpsListActivity.this, "获取GPS数据失败", IBase.RETAIL_ZERO);
                }
            }
            ...
            public void onFailure(int errorNo, String strMsg) {
                ...
                refreshLayout_bindings.finishRefreshing();
                LogTxt.getInstance().writeLog("获取GPS绑定列表失败，错误编码：" + errorNo + "，错误信息：" + strMsg);
                if (errorNo != 404)
                    IBaseMethod.showToast(GpsListActivity.this, strMsg, IBase.RETAIL_ZERO);
            }
        });
    }
        ...
        if (v.getId() == R.id.delete) {
            DialogUtil.prompt(this, "确认删除?", "取消", "删除", new DialogButtonListener() {
                ...
                public void ok() {
                    ...
                    gpsAdapter.updateSelected(flag);
                }
            ...
            Intent intent = new Intent(GpsListActivity.this, GpsActivity.class);
